...
type decapConfigReloader func(cfg *Config) error

func NewDecapModule(cfg *Config, configReloader decapConfigReloader, log *zap.SugaredLogger) (*DecapModule, error) {
	...
	decapService := NewDecapService(cfg, configReloader, agents, log)
...
func (m *DecapModule) Run(ctx context.Context) error {
	...
	// First, update module configurations without locking,
	// as the service is not running yet.
	// TODO: iterate over all available modules
	if err := m.decapService.updateModuleConfigs("decap0", nil, m.cfg.Prefixes); err != nil {
		return fmt.Errorf("failed to update module config: %s: %w", "decap0", err)
	}
