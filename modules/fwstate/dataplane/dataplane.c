...
is_fw_state_sync_packet(
	struct packet *packet, struct fw_state_sync_config *sync_config
) {
...
fwstate_build_value(
	...
	bool is_external,
	uint64_t now,
	struct fw_state_timeouts *timeouts_config
) {
...
fwstate_process_sync_v4(
	...
	bool is_external,
	uint64_t now,
	struct fw_state_timeouts *timeouts
) {
...
fwstate_handle_packets(
	...
	struct fwstate_config *fwstate_config =
		ADDR_OF(&fwstate_module->fwstate_cfg);
