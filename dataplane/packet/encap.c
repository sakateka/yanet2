	...
	const size_t size)
{
	...
	memcpy(
		rte_pktmbuf_mtod_offset(mbuf, char*, packet->network_header.offset),
		header,
		size);
	...
	uint16_t* nextHeaderType = rte_pktmbuf_mtod_offset(mbuf, uint16_t*, packet->network_header.offset - 2);
	*nextHeaderType = type;
}
	...
	const uint8_t *src)
{
	...
	if (ipv4HeaderInner) {
		...
		header.total_length = rte_cpu_to_be_16(sizeof(struct rte_ipv4_hdr) + rte_be_to_cpu_16(ipv4HeaderInner->total_length));
		...
		header.type_of_service = (rte_be_to_cpu_32(ipv6HeaderInner->vtc_flow) >> 20) & 0xFF;
		header.total_length = rte_cpu_to_be_16(sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_ipv6_hdr) + rte_be_to_cpu_16(ipv6HeaderInner->payload_len));
	...
	const uint8_t *src)
{
	...
	if (ipv4HeaderInner != NULL) {
		...
		header.payload_len = rte_cpu_to_be_16(sizeof(struct rte_ipv6_hdr) + rte_be_to_cpu_16(ipv6HeaderInner->payload_len));
...
packet_gre4_encap(
	...
	const uint8_t *src)
{
	if (packet_ip4_encap(packet, dst, src))
		return -1;
...
packet_gre6_encap(
	...
	const uint8_t *src)
{
	if (packet_ip6_encap(packet, dst, src))
		return -1;
