...
module_config_registry_lookup(
	...
	for (uint32_t idx = 0; idx < module_config_registry->config_count;
	     ++idx) {
		if (!strncmp(
			    module_config_name,
...
module_registry_configure(
	...
	for (uint32_t idx = 0; idx < module_config_registry->config_count;
		...
		struct module_config **config =
			module_config_registry->configs + idx;
		if (!strncmp(
			    module_config_name,
			    (*config)->name,
			    MODULE_CONFIG_NAME_LEN
	...
	if (module_config_registry->config_count % 8 == 0) {
		struct module_config **new_configs = (struct module_config **)
			realloc(module_config_registry->configs,
	...
	struct module_config **config = module_config_registry->configs +
