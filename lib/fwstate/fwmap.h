...
static inline void
fwmap_get_stats(const fwmap_t *map, fwmap_stats_t *stats) {
	if (!stats)
		return;

	memset(stats, 0, sizeof(fwmap_stats_t));
	stats->total_elements = fwmap_size(map);
	stats->index_size = map->index_mask + 1;
	stats->max_chain_length = fwmap_max_chain_length(map);
...
fwmap_new(const fwmap_config_t *user_config, struct memory_context *ctx) {
	...
	// FIXME: ensure cache line alignment
	fwmap_t *map = memory_balloc(ctx, map_size);
	if (!map) {
		errno = ENOMEM;
		return NULL;
	}
	...
	for (uint32_t i = 0; i < chunk_count; i++) {
		// FIXME: ensure cache line alignment
		fwmap_bucket_t *chunk = memory_balloc(ctx, index_chunk_size);
		if (!chunk) {
