...
ttlmap_get_stats(const ttlmap_t *map, ttlmap_stats_t *stats) {
	...
	uint32_t chunk_count =
		(map->chunks_mask >> map->buckets_chunk_shift) + 1;
...
ttlmap_allocate_chunks(
	...
	for (uint32_t i = 0; i < chunks; i++) {
		...
		/*
		 * Memory initialization is intentionally omitted here for
		 * performance:
		 * - The allocated memory will be used for key/value storage
		 * - All slots are initially considered "free" (uninitialized)
		 * - The map tracks which slots contain valid data via bucket
		 * metadata (specifically via sig[], deadline[], and idx[]
		 * arrays in ttlmap_bucket_t)
		 * - Each slot will be fully overwritten before being read,
		 * ensuring no uninitialized memory access occurs
		 * - This optimization avoids unnecessary memset() on
...
ttlmap_new(
	...
	if (index_size < 16)
		index_size = 16;
	index_size =
		align_up_pow2(index_size); // Ensure index_size is a power of 2
	if (!index_size) {
		errno = EINVAL;
		return NULL;
	}
	...
	map->chunks_mask = map->index_mask & ~TTLMAP_CHUNK_INDEX_MASK;
	...
	if (extra_size > 0) {
		...
		// Memory initialization is intentionally omitted here for
		// performance: The allocated memory will be used for extra
		// buckets that are initialized before use
...
ttlmap_get(
	...
	uint32_t chunk_idx =
		(hash & map->chunks_mask) >> map->buckets_chunk_shift;
...
ttlmap_update_counters(
	...
	if (counter->gen != map->gen) {
		// Reset counters after gen change
		counter->gen = map->gen;
...
ttlmap_put(
	...
	if (bucket->gen != map->gen) {
		memset(&bucket->sig, 0, sizeof(bucket->sig));
		bucket->gen = map->gen;
		bucket->next = 0;
	}
	...
	// Update counters
	chain_length++;
	ttlmap_update_counters(map, worker_idx, chain_length, 1, now + 1);
