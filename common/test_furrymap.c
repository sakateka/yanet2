/**
 * @file test_furrymap.c
 * @brief Comprehensive test program for FurryMap implementation
 */
...
/**
 * @brief Allocate memory using mmap with mlock for better performance
 */
static void *
allocate_locked_memory(size_t size) {
	...
	if (mlock(arena, size) != 0) {
		perror("mlock failed");
		// Continue anyway as mlock failure is not critical
	}
 ...
 */
static void
free_locked_memory(void *ptr, size_t size) {
...
static inline void
init_default_config(struct memory_context *ctx, furrymap_config_t *config) {
	SET_OFFSET_OF(&config->mem_ctx, ctx);
	config->key_size = sizeof(int);
	config->value_size = sizeof(int);
	config->hash_fn_id = FURRYMAP_HASH_FNV1A;
	config->key_equal_fn_id = FURRYMAP_KEY_EQUAL_DEFAULT;
	config->rand_fn_id = FURRYMAP_RAND_DEFAULT;
	config->hash_seed = 0;
	config->worker_count = 1;
	config->enable_locks = false;
}
...
test_basic_operations(void *arena) {
	...
	furrymap_config_t config = {0};
	init_default_config(ctx, &config);
	config.hash_seed = 0x12345678;

	printf("L%d: FurryMap new()\n", __LINE__);
	furrymap_t *map = furrymap_new(&config, 64);
	assert(map != NULL);

	printf("L%d: FurryMap empty()\n", __LINE__);
	bool is_empty = furrymap_empty(map);
	assert(is_empty);

	printf("L%d: FurryMap size()\n", __LINE__);
	size_t size = furrymap_size(map);
	assert(size == 0);

	// Test insertion
	int key1 = 777, value1 = 100;
	printf("L%d: FurryMap put()\n", __LINE__);
	bool put_ok = furrymap_put(map, &key1, &value1, worker_idx);
	assert(put_ok);

	printf("L%d: FurryMap size()\n", __LINE__);
	size = furrymap_size(map);
	assert(size == 1);

	printf("L%d: FurryMap empty()\n", __LINE__);
	is_empty = furrymap_empty(map);
	assert(!is_empty);

	// Test retrieval
	int *found_value = NULL;
	printf("L%d: FurryMap get()\n", __LINE__);
	bool get_ok = furrymap_get(map, &key1, (void **)&found_value);
	assert(get_ok);
	...
	int value2 = 200;
	printf("L%d: FurryMap put()\n", __LINE__);
	put_ok = furrymap_put(map, &key1, &value2, worker_idx);
	assert(put_ok);
	...
	printf("L%d: FurryMap get()\n", __LINE__);
	get_ok = furrymap_get(map, &key1, (void **)&found_value);
	assert(get_ok);
	assert(*found_value == 200);

	// Test multiple insertions
	printf("L%d: FurryMap put() +100 values\n", __LINE__);
	for (int i = 0; i < 100; i++) {
		int key = i;
		int value = i * 10;
		int ok = furrymap_put(map, &key, &value, worker_idx);
		assert(ok);
		size = furrymap_size(map);
	...
	for (int i = 0; i < 100; i++) {
		int key = i;
		get_ok = furrymap_get(map, &key, (void **)&found_value);
		if (!get_ok) {
	...
	// Test deletion
	bool delete_ok = furrymap_delete(map, &key1);
	assert(delete_ok);
	size = furrymap_size(map);
	assert(size == 100);
	get_ok = furrymap_get(map, &key1, (void **)&found_value);
	...
	int non_existent = 9999;
	delete_ok = furrymap_delete(map, &non_existent);
	assert(!delete_ok);
	size = furrymap_size(map);
	assert(size == 100);
...
test_collision_handling(void *arena) {
	...
	furrymap_t *map = furrymap_new(&config, 16);
	assert(map != NULL);
	...
	for (int i = 0; i < 1000; i++) {
		int key = i;
		int value = i * 2;
		bool put_ok = furrymap_put(map, &key, &value, worker_idx);
		assert(put_ok);
	}
	...
	for (int i = 0; i < 1000; i++) {
		int key = i;
		int *found_value = NULL;
		bool get_ok = furrymap_get(map, &key, (void **)&found_value);
		assert(get_ok);
		assert(*found_value == i * 2);
	}

	// Check chain length statistics
	printf("  Max chain length: %zu\n", furrymap_max_chain_length(map));

	furrymap_stats_t stats;
	furrymap_get_stats(map, &stats);
	printf("  Total groups: %zu\n", stats.total_groups);
	printf("  Total subgroups: %zu\n", stats.total_subgroups);
...
benchmark_performance(void *arena) {
	...
	const int num_items = TOTAL_VALUES;
	const int index_size = TOTAL_VALUES / 4;
	furrymap_t *map = furrymap_new(&config, index_size);
	assert(map != NULL);
	...
	for (int i = 0; i < num_items; i++) {
		...
		bool put_ok = furrymap_put(map, &key, &value, worker_idx);
		if (!put_ok) {
			printf("failed to insert value %d at L%d", i, __LINE__);
			exit(1);
		}
	}
	double end = get_time();
	size_t total_elements = map->total_elements;
	assert(total_elements == (size_t)num_items);
	double insert_time = end - start;
	double insert_throughput = num_items / insert_time;
	...
	for (int i = 0; i < num_items; i++) {
		int key = i;
		int *value;
		bool get_ok = furrymap_get(map, &key, (void **)&value);
		if (!get_ok) {
			printf("failed to get key: %d at L%d", key, __LINE__);
			exit(1);
		}
		checksum +=
			*value; // Force compiler to actually use the read value
	}
	end = clock();
	...
	double lookup_throughput = num_items / lookup_time;
	...
	printf("    Total groups: %s\n", numfmt(stats.total_groups));
	printf("    Total subgroups: %s\n", numfmt(stats.total_subgroups));
...
writer_thread(void *arg) {
	...
	for (; j < NUM_REPETITIONS; j++) {
		for (size_t i = 0; i < TOTAL_VALUES; i++) {
			...
			bool put_ok = furrymap_put_safe(
				data->map, &key, value_buffer, data->thread_id
			);
			if (put_ok) {
				successful++;
				if (j == 0) {
...
test_multithreaded_benchmark(void) {
	...
	size_t index_size = TOTAL_VALUES / 8;
	...
	if (!map) {
		if (errno != 0) {
			perror("failed to create FurryMap: ");
		} else {

			printf("Failed to create FurryMap (unknown error)\n");
		}
		free_locked_memory(mt_arena, MT_ARENA_SIZE);
		exit(1);
	}
	...
	printf("Write throughput: " COLOR_CYAN "%s ops/sec" COLOR_RESET "\n",
	       numfmt((size_t)TOTAL_OPS / total_write_elapsed_time));
	...
	printf("Read throughput: " COLOR_CYAN "%s ops/sec" COLOR_RESET "\n",
	       numfmt((size_t)TOTAL_OPS / total_read_elapsed_time));
...
main() {
	printf(COLOR_BOLD COLOR_WHITE
	       "=== FurryMap Comprehensive Test Suite ===" COLOR_RESET "\n\n");
	...
	free_locked_memory(arena, ARENA_SIZE);
	printf("\n" COLOR_BOLD COLOR_GREEN
