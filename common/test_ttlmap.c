...
static inline void
init_default_config(ttlmap_config_t *config) {
...
test_basic_operations(void *arena) {
	...
	ttlmap_t *map = ttlmap_new(&config, ctx, 128, 8);
	assert(map != NULL);
...
test_collision_handling(void *arena) {
	...
	ttlmap_t *map = ttlmap_new(&config, ctx, 1000, 1000);
	assert(map != NULL);
...
benchmark_performance(void *arena) {
	...
	const int index_size = L3_CACHE_SIZE / config.value_size / 2;
	ttlmap_t *map = ttlmap_new(&config, ctx, index_size, index_size >> 8);
	assert(map != NULL);
